(function () {
    ...
    function AdminGrowthPartnerReportsItemController($scope, $state, partnerReportService, pageService, allPartners, adminUserService, NgTableParams, $mdToast, $q, Restangular, $mdDialog, $timeout, $rootScope, commonDialogService, $stateParams, toaster, reportService, actionplanService, $window) {
        ...
        pageService
            .reset() 
            .setShowBC(true)
            .addCrumb({name: 'Growth Partner Reports', path: 'reports.partner.item'})
            .setPageTitle('Growth Partner Reports');
        ...
        function buildReport() {
            ...
            if ($scope.partner._id && startDate && endDate){
                return partnerReportService.postGrowthReport({partnerId: $scope.partner._id, from: startDate, to: endDate})
